    ...
    def learn_policy(
        ...
        for i in range(num_episodes):
            ...
            while True:
                ...
                # SARSA update rule with eligibility traces
                td_error = reward + self.gamma * self.qtable[next_state, next_action] - self.qtable[state, action]
                self.etable[state, action] += 1  # Accumulate eligibility trace
                self.qtable += self.alpha * td_error * self.etable  # Update Q-table with eligibility traces
                self.etable *= self.gamma * self.lambda_value  # Decay eligibility traces
...
def render_visualization(learned_policy):
    ...
    env.render()
    while True:
        next_state, reward, done, _, info = env.step(learned_policy[state, 0])
        env.render()
